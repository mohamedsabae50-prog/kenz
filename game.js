...
function spawnLevel() {
    ...
    while (currentX < desiredLength) {
        ...
        // إصلاح النطات: أقصى طلوع لفوق هو 110 بيكسل (عشان اللاعب يقدر ينطها) وأقصى نزول 180
        let yShift = (Math.random() * 290) - 110; 
        currentY = clamp(previousPlatform.y + yShift, -800, canvas.height - 100); 

        // المسار الأساسي (السفلي)
        ...
        // المسار العلوي (منصات طائرة)
        if (Math.random() > 0.45) {
            platforms.push({ 
                x: currentX + (Math.random() * 40 - 20), 
                y: currentY - 180 - (Math.random() * 50), // أعلى بمسافة تسمح بالنط
                width: platWidth * 0.8,
                height: 30, 
                hasStrawberry: false, 
                isUpper: true // علامة عشان نرسمها كمنصة طايرة مش عمود
            });
        }
...
resizeCanvas(); // استدعائها أول مرة
...
function update(deltaTime) {
    ...
    // التحقق من وجود اللاعب داخل المياه
    let inWater = false;
    ...
    let currentFriction = inWater ? 0.65 : FRICTION; // الماية بتبطأ الحركة الأفقية شوية
    let currentGravity = inWater ? 0.2 : GRAVITY;    // الماية بتقلل الجاذبية (طفو)
    ...
    if (isJumpKeyPressed && !player.jumpLock) {
        if (inWater) { 
            // العوم في الماية (نط لا نهائي بقوة أقل)
    ...
    let maxFallSpeed = inWater ? 5 : 20; // السقوط في الماية بطيء
    ...
    // التعديل هنا: هنخلي اللاعب في منتصف الشاشة الرأسي (مقسومة على 2)
    // كده هتشوف نص الشاشة فوقك، ونص الشاشة بالكامل تحتك!
    let targetCameraY = player.y - (canvas.height / 2); 

    cameraX += (targetCameraX - cameraX) * 0.1;
    
    // سرعتها شوية صغيرة (0.08 بدل 0.03) عشان تلحقك وانت بتقع لتحت بسرعة
    cameraY += (targetCameraY - cameraY) * 0.08; 

    if (cameraX < 0) cameraX = 0;
    
    // عشان الكاميرا ماتنزلش تحت الأرض بزيادة لو مت
    if (cameraY > levelDeathY - canvas.height) {
        cameraY = levelDeathY - canvas.height;
    }
...
function drawGoalFallback(x, y, w, h) {
    ctx.save();
    ctx.fillStyle = '#bdbdbd'; ctx.fillRect(x + w/2 - 5, y, 10, h); 
    ctx.fillStyle = '#e91e63'; ctx.beginPath(); ctx.moveTo(x + w/2, y); ctx.lineTo(x + w, y + 20); ctx.lineTo(x + w/2, y + 40); ctx.fill(); 
    ctx.fillStyle = '#d7ccc8'; ctx.beginPath(); ctx.arc(x + w/2, y + h - 15, 20, 0, Math.PI); ctx.fill();
    ctx.fillStyle = '#ff1744'; ctx.beginPath(); ctx.arc(x + w/2 - 5, y + h - 20, 8, 0, Math.PI*2); ctx.arc(x + w/2 + 5, y + h - 22, 8, 0, Math.PI*2); ctx.arc(x + w/2, y + h - 25, 8, 0, Math.PI*2); ctx.fill();
    ctx.restore();
}
...
function draw() {
    ...
    // تطبيق حركة الكاميرا الأفقية والرأسية
    ...
    platforms.forEach(plat => {
        // لو دي منصة علوية (isUpper)، هنخلي طولها الحقيقي بس، لو منصة سفلية، ننزلها لـ 3000 بيكسل كعمود
        ...
        if (images.ground.complete && images.ground.naturalWidth > 0) {
            ...
            ctx.fillRect(0, 0, plat.width, drawHeight); 
            ctx.restore();
        } else { 
            // Fallback
            ctx.fillStyle = '#5d4037'; 
            ctx.fillRect(plat.x, plat.y, plat.width, drawHeight); 
